...
//console.log("ma ssssss");
const set_erc_TLD = async (id,address) => {
  //console.log("Set domain ID:"+id+" Price: "+price);
  console.log("Set ERC Token for "+id+" with address "+address);
  try {
    const result = await contract.methods
      .set_erc_TLD(id,address)
      .send({ from: account,maxPriorityFeePerGas: null,  maxFeePerGas: null, });
    console.log(result);
    return result.status;
  } catch (error) {
...
//Get ERC bind
const get_erc_TLD = async (id) => {
  try {
    const did = await contract.methods.get_erc_TLD(id).call();
    return did;
  } catch (error) {
    console.log(error.message);
  }
...
//Get TLD Commission
const get_Comm_TLD = async (id) => {
  try {
    const did = await contract.methods.get_Comm_TLD(id).call();
    return did;
  } catch (error) {
    console.log(error.message);
  }
